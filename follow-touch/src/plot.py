...
class Plot(QWidget):
    def __init__(self, number_of_sensors=0, parent=None):
        ...
        # settings per sensor
        self._setting = []
        self._setting.append(
            {
                "color": QColor(0, 0, 255, 127),
                "color_bias": QColor(0, 2, 0, 127),
                "grid_position": [0, 0],
            }
        )
        self._setting.append(
        ...
        self.x = []
        self.y = []
        for _ in range(self._number_of_sensors):
            self.x.append(0)
            self.y.append(0)
        self.x_bias = []
        self.y_bias = []
    ...
    def closeEvent(self, _):
        try:
            ...
            self.bleServiceManager.connection_dict.__setitem__(
                self.bleServiceManager.device_name, "PlotWindowClosed"
            )
        except:
            pass
    ...
    def info(self, ble_service_manager=None):
        ...
        self.sensor_device_name = (
            self.bleServiceManager.device_name or self.sensor_device_name
        )
        ...
        self._screen_positions.append(
            [self.top_x + self.width + self.gap_x, self.top_y, self.width, self.height]
        )
        self._screen_positions.append(
            [self.top_x, self.top_y + self.height + self.gap_y, self.width, self.height]
        )
        self._screen_positions.append(
            [
                self.top_x + self.width + self.gap_x,
                self.top_y + self.height + self.gap_y,
                self.width,
                self.height,
            ]
        )
        try:
            self._screen_position = self._screen_positions[
                int(self.bleServiceManager.device_name[-1:])
            ]  # last charachter
            self.setGeometry(
                self._screen_position[0],
                self._screen_position[1],
                self._screen_position[2],
                self._screen_position[3],
            )
        except:
            self._screen_position = None
    ...
    def initUI(self):
        ...
        for i in range(0, self._number_of_sensors):
            ...
            self.layout.addWidget(
                self.ellipse_widgets[i], grid_position_x, grid_position_y
            )
    ...
    def update_ellipse_from_external_source(self):
        ...
        for i in range(0, self._number_of_sensors):
            new_rect = QRect(
                self.ellipse_widgets[i].x() - int(self.x[i] / 2),
                self.ellipse_widgets[i].y() - int(self.y[i] / 2),
                self.x[i],
                self.y[i],
            )
            new_color = QColor(self._setting[i].get("color"))
            new_rect_bias = QRect(
                self.ellipse_widgets[i].x() - int(self.x_bias[i] / 2),
                self.ellipse_widgets[i].y() - int(self.y_bias[i] / 2),
                self.x_bias[i],
                self.y_bias[i],
            )
            new_color_bias = QColor(self._setting[i].get("color_bias"))
            self.ellipse_widgets[i].update_ellipse(
                new_rect, new_color, new_rect_bias, new_color_bias
            )
    ...
    def _data_to_xy(self, data):
        try:
            ...
            for sensor_data in sensor_data_list:
                ...
                sensor_list.append(())
                sensor_list[int(sensor_data.get("s"))] = (
                    int(sensor_data.get("i")),
                    int(sensor_data.get("b")),
                )

                self.y[index] = (
                    sensor_list[index][0] - sensor_list[index][1]
                )  # sensor index with values on-off in a tuple
                self.x[index] = (
                    sensor_list[index][0] - sensor_list[index][1]
                )  # sensor index with values on-off in a tuple

                if self.bias:
                    self.y[index] = sensor_list[index][
                        0
                    ]  # sensor index with values on-off in a tuple
                    self.x[index] = sensor_list[index][
                        0
                    ]  # sensor index with values on-off in a tuple
                    self.y_bias[index] = sensor_list[index][
                        1
                    ]  # sensor index with values on-off in a tuple
                    self.x_bias[index] = sensor_list[index][
                        1
                    ]  # sensor index with values on-off in a tuple

                self.y[index] = int(self.factor * (self.y[index] * self.height / 4096))
                self.x[index] = int(self.factor * (self.x[index] * self.width / 4096))
                self.y_bias[index] = int(
                    self.factor * (self.y_bias[index] * self.height / 4096)
                )
                self.x_bias[index] = int(
                    self.factor * (self.x_bias[index] * self.width / 4096)
                )
                index += 1
...
"""
    data = []
    data.append(
        """{"t":{"000106":[{"s":"0","i":"2202","b":"0195"},{"s":"1","i":"2342","b":"0435"},{"s":"2","i":"1365","b":"0247"},{"s":"3","i":"1483","b":"0295"}]}}"""
    )
    data.append(
        """{"t":{"000205":[{"s":"0","i":"2212","b":"0279"},{"s":"1","i":"2333","b":"0549"},{"s":"2","i":"1361","b":"0303"},{"s":"3","i":"1471","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000305":[{"s":"0","i":"2222","b":"0279"},{"s":"1","i":"2353","b":"0549"},{"s":"2","i":"1371","b":"0303"},{"s":"3","i":"1421","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000405":[{"s":"0","i":"2232","b":"0279"},{"s":"1","i":"2323","b":"0549"},{"s":"2","i":"1391","b":"0303"},{"s":"3","i":"1751","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000505":[{"s":"0","i":"2242","b":"0279"},{"s":"1","i":"2313","b":"0549"},{"s":"2","i":"1331","b":"0303"},{"s":"3","i":"1471","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000605":[{"s":"0","i":"4252","b":"0549"},{"s":"1","i":"4252","b":"0549"},{"s":"2","i":"1341","b":"0303"},{"s":"3","i":"1491","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000705":[{"s":"0","i":"4262","b":"0549"},{"s":"1","i":"4262","b":"0549"},{"s":"2","i":"1371","b":"0303"},{"s":"3","i":"1471","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000805":[{"s":"0","i":"2272","b":"0279"},{"s":"1","i":"2353","b":"0549"},{"s":"2","i":"1321","b":"0303"},{"s":"3","i":"1461","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"000905":[{"s":"0","i":"2282","b":"0279"},{"s":"1","i":"2343","b":"0549"},{"s":"2","i":"1311","b":"0303"},{"s":"3","i":"1431","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"001005":[{"s":"0","i":"2292","b":"0279"},{"s":"1","i":"2333","b":"0549"},{"s":"2","i":"1361","b":"0303"},{"s":"3","i":"1421","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"001105":[{"s":"0","i":"2342","b":"0279"},{"s":"1","i":"2383","b":"0549"},{"s":"2","i":"1371","b":"0303"},{"s":"3","i":"1411","b":"0386"}]}}"""
    )
    data.append(
        """{"t":{"001205":[{"s":"0","i":"2322","b":"0279"},{"s":"1","i":"2363","b":"0549"},{"s":"2","i":"1381","b":"0303"},{"s":"3","i":"1431","b":"0386"}]}}"""
    )
