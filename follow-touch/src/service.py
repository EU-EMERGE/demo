...
class BLEServiceManager():
    def __init__(self):
        ...
        self.follow_touch_command_characteristic_uuid = "{b47b6628-5fc7-4140-86c1-45f9a9be9fbc}"
        self.follow_touch_result_characteristic_uuid = "{fda4cdfd-0575-4f78-86d1-7de26458e0cf}"
    ...
    def do_service(self, device_info, connection_dict):
        if device_info.address().toString():
            ...
            if not self.ble_controller:
                ...
                self.ble_controller.discoveryFinished.connect(self.service_discovery_finished)
...
#-- Controller signals start ---------------------------------------------------------------------------------------------------
    ...
    def service_discovered(self, service_uuid):
        ...
        if service_uuid.toString().__contains__("00001849-0000-1000-8000-00805f9b34fb"):
            ...
            self.ble_service = self.ble_controller.createServiceObject(QUuid(service_uuid.toString()))
            if self.ble_service:
                # Define signals
                self.ble_service.characteristicChanged.connect(self.characteristic_changed)
                self.ble_service.characteristicRead.connect(self.characteristic_read)
                self.ble_service.characteristicWritten.connect(self.characteristic_written)
                ...
                self.ble_service.discoverDetails(mode=QLowEnergyService.DiscoveryMode.SkipValueDiscovery)            
...
#-- Service signals start ---------------------------------------------------------------------------------------------------
    def service_state_changed(self, state):
        ...
        if state == QLowEnergyService.ServiceState.RemoteServiceDiscovered:
            for _characteristic in self.ble_service.characteristics():
                if _characteristic.uuid().toString() == self.follow_touch_command_characteristic_uuid:
                    self.follow_touch_command_characteristic = _characteristic
                elif _characteristic.uuid().toString() == self.follow_touch_result_characteristic_uuid:
                    self.follow_touch_result_characteristic = _characteristic

            # make sure notification is on for both used characteristics
            characteristic_command = self.ble_service.characteristic(QUuid(self.follow_touch_command_characteristic_uuid))
            if characteristic_command.isValid():
                self.enable_notifications(characteristic_command)
            else:
                print("invalid")

            characteristic_result = self.ble_service.characteristic(QUuid(self.follow_touch_result_characteristic_uuid))
            ...
            try:
                self.ble_service.writeCharacteristic(self.follow_touch_command_characteristic, 'standby'.encode("utf8"), mode=QLowEnergyService.WriteWithResponse)
            except:
                pass
    ...
    def characteristic_changed(self, info, value):
        ...
        try:
            ...
            self.plot.draw(value.data().decode("utf8")[:-2])  # minus last 2 characters (,\n)
...
#-- Service signals end ---------------------------------------------------------------------------------------------------
    ...
    def enable_notifications(self, characteristic):
        cccd = characteristic.descriptor(QBluetoothUuid.DescriptorType.ClientCharacteristicConfiguration)
        if cccd.isValid():
...
#-- Act on user interface action -------------------------------------------------------------------------------------------
    def start_measurement(self, measurement_pace, file_output=False, filename=None):
        ...
        if self.filename != None:    # got a name in do_service method
            ...
            if filename != None:
                ...
                self.full_filename = "%s%s%s" % (dirname, os.path.sep, "%s_%s.json" % (self.filename, basename))      
        ...
        self.ble_service.writeCharacteristic(self.follow_touch_command_characteristic, start_command.encode("utf8"), mode=QLowEnergyService.WriteWithResponse)

    def stop_measurement(self):
        print("stop command")
        try:
            self.ble_service.writeCharacteristic(self.follow_touch_command_characteristic, 'stop'.encode("utf8"), mode=QLowEnergyService.WriteWithResponse)
        except:
            pass
        if self.file_output == True:
            self.stop_file()
            try:
                follow_touch_conversion = FollowTouchConversion(json_file=self.full_filename)
    ...
    def start_file(self):
        ...
        start_json = '{"%s":[' % self.filename        # start of json file with basename of written file
        self.filehandler.write(start_json)

    def stop_file(self):
        try:
            # remove last characters (\n and comma)
            self.filehandler.seek(0, os.SEEK_END)                   # seek to end of file; f.seek(0, 2) is legal
            self.filehandler.seek(self.filehandler.tell()-2, os.SEEK_SET) # go backwards 2 bytes
