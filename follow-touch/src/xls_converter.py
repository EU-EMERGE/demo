...
class FollowTouchConversion:
        ...
        for number in range(0, len(self.valid_filenumbers)):
            path = os.sep.join(
                (
                    self.parts.get("dir"),
                    "%s_%s_%s.json"
                    % (self.parts.get("name"), number, self.parts.get("time")),
                )
            )
            if self.file_exist(path):
                print(path)
                self.workbook.create_sheet(
                    "%s_%s" % (self.parts.get("name"), number), int(number)
                )
    ...
    def _write(self, wb):
        outfile = os.sep.join(
            (
                self.parts.get("dir"),
                "%s_%s.xls" % (self.parts.get("name"), self.parts.get("time")),
            )
        )
        wb.save(outfile)
...
if __name__ == "__main__":
    filename = os.sep.join(
        ("/home", "andre", "output", "follow_touch_0_20240813_154021.json")
    )
    print(filename)
